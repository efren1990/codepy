...
def index():
	...
	# Consulta sql
	miCur.execute("SELECT * FROM contactos")
...
def contactos():
	...
	if request.method == 'POST':
		...
		miCur.execute("INSERT INTO contactos(nombre, telefono, email) VALUES(%s, %s, %s)", (nom, tel, ema))
...
def editar_con(idEdit):
	...
	miCur = mysql.connection.cursor()
	# Consulta de datos
	miCur.execute("SELECT * FROM contactos WHERE id_cont = {0}".format(idEdit))
	# confirmar cambios
	dato = miCur.fetchall()
...
def eliminar_con(idElim):
	...
	miCur = mysql.connection.cursor()
	# Consulta
	miCur.execute("DELETE FROM contactos WHERE id_cont = {0}".format(idElim))
